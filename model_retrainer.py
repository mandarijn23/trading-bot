...
class ModelRetrainer:
    ...
    def load_trade_history(self) -> None:
        ...
        try:
            ...
            self.closed_trades_count = closed_trades
            self.trades_since_retrain = max(0, self.closed_trades_count - self.last_closed_trades_seen)
    ...
    def prepare_training_data(self) -> tuple | None:
        """
        ...
        for trade in recent:
            if "pnl_pct" in trade and trade["pnl_pct"]:
                try:
                    ...
                    y.append(label)
                    X.append([
                        float(str(trade.get("ai_confidence", "50")).strip("%")) / 100,
                        pnl_pct,
                    ])
                except Exception:
                    pass
...
class TradeAnalytics:
    ...
    def get_daily_stats(df: pd.DataFrame) -> dict:
        ...
        for date, group in df.groupby("date"):
            ...
            wins = len(
                group[
                    group["pnl_pct"].str.contains(r"^-?[0-9]")
